  ...
  return (
    <div className="p-8 max-w-6xl mx-auto animate-slideUp">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold mb-1" style={{ color: 'var(--text-primary)' }}>{t('assetLibrary')}</h1>
          <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>{t('totalAssets')}: {assets.length}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <button onClick={loadAssets} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: 'var(--accent)' }}>{t('retry')}</button>
        </div>
      ) : filteredAssets.length === 0 ? (
        <div className="text-center py-16">
          <Image size={40} className="mx-auto mb-3" style={{ color: 'var(--border-hover)' }} />
          <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>{t('noAssets')}</p>
        </div>
      ) : (
            ...
            <div key={asset.id} className="group surface overflow-hidden transition-all duration-200 hover:border-[var(--border-hover)]" style={{ border: '1px solid var(--border)' }}>
              <div className="aspect-square overflow-hidden relative">
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                  <a href={asset.url} target="_blank" rel="noreferrer" download className="w-8 h-8 rounded-full bg-white flex items-center justify-center">
              ...
              <div className="p-2.5">
                <div className="flex items-center gap-1.5" style={{ color: 'var(--text-tertiary)' }}>
                  ...
                  <span className="text-[11px] uppercase font-semibold">{asset.type}</span>
                </div>
              </div>
            </div>
          ))}
